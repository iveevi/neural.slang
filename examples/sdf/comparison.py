...
# TODO: use decorators to shorten the pipeline class description...
class Pipeline:
    # @specialization
    @staticmethod
    ...
    # @compute_pipeline("render_neural")
...
# TODO: use the same training pipeline for all examples...
...
def main():
    ...
    pytorch = PyTorchNetwork(hidden=hidden, hidden_layers=hidden_layers, levels=0, input=3, output=1)
    ...
    optimizer = torch.optim.Adam(pytorch.parameters(), lr=1e-3, betas=(0.9, 0.999), eps=1e-8)
    ...
    while not window.should_close():
        ...
        # Training PyTorch
        samples_pytorch = torch.from_numpy(samples).float()
        sdf_pytorch = pytorch.forward(samples_pytorch)
        sdf_target = torch.from_numpy(sdf).float().unsqueeze(-1)
        loss_pytorch = (sdf_pytorch - sdf_target).square().mean()
        record_loss_pytorch = (sdf_pytorch - sdf_target).abs().mean()
        pytorch_history.append(record_loss_pytorch.item())
    ...
    pytorch_history = np.array(pytorch_history)

    sns.lineplot(history, alpha=0.5, color="green")
    sns.lineplot(pytorch_history, alpha=0.5, color="orange")

    sns.lineplot(gaussian_filter(history, 5), linewidth=2.5, label="Slang", color="green")
    sns.lineplot(gaussian_filter(pytorch_history, 5), linewidth=2.5, label="PyTorch", color="orange")
