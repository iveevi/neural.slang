...
def linear_gradients_to_bindless_numpy(linear: nn.Linear, dtype: np.dtype = np.float32) -> tuple[np.ndarray, np.ndarray]:
    assert linear.weight.grad is not None
    assert linear.bias.grad is not None
    weights_grad = linear.weight.grad.cpu().detach().numpy().T.ravel().astype(dtype)
    bias_grad = linear.bias.grad.cpu().detach().numpy().ravel().astype(dtype)
    return np.ascontiguousarray(weights_grad), np.ascontiguousarray(bias_grad)