...
def test_sgd(device, make_kernel, seed, num_params, lr, momentum):
    ...
    parameters_buffer = create_buffer_32b(device, initial_params.copy())
    state_buffer = create_buffer_32b(device, sgd_state_data)  # 1 float per state
    ...
    for iteration in range(num_iterations):
        ...
        gradients_buffer = create_buffer_32b(device, gradients)
...
def test_adam(device, make_kernel, seed, num_params):
    ...
    parameters_buffer = create_buffer_32b(device, initial_params.copy())
    state_buffer = create_buffer_32b(device, adam_state_data, 3)  # 3 floats per state
