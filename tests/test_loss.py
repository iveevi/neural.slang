...
def test_mse(device, make_kernel, random_seed, in_size):
...
@pytest.mark.parametrize("random_seed", [0])
@pytest.mark.parametrize("in_size", [16, 32, 64, 128])
def test_mae(device, make_kernel, random_seed, in_size):
    np.random.seed(random_seed)
    ...
    specialization_module = create_specialization_module(device, in_size)
    kernel = make_kernel("mae", link_modules=[specialization_module])
    ...
    output = output_buffer.to_numpy().view(np.float32)
    expected = np.mean(np.abs(input_data - target_data), axis=1)
...
@pytest.mark.parametrize("random_seed", [0])
@pytest.mark.parametrize("in_size", [16, 32, 64, 128])
def test_mae_derivative(device, make_kernel, random_seed, in_size):
    np.random.seed(random_seed)
    ...
    specialization_module = create_specialization_module(device, in_size)
    kernel = make_kernel("mae_derivative", link_modules=[specialization_module])
    ...
    # Use PyTorch autograd to compute MAE derivative
    predicted_torch = torch.tensor(predicted_data, requires_grad=True)
    expected_torch = torch.tensor(expected_data)
    
    # Compute MAE loss
    mae_loss = torch.mean(torch.abs(predicted_torch - expected_torch), dim=1)
    # Sum over batch dimension to get scalar loss for backprop
    total_loss = torch.sum(mae_loss)
