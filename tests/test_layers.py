...
@pytest.mark.parametrize("random_seed", RANDOM_SEEDS)
def test_feed_forward_basic_nn_linear(device, make_kernel, random_seed):
    """Test feed forward layer using nn.Linear instead of manual matrix operations."""
    kernel = make_kernel("feed_forward")
    np.random.seed(random_seed)
...
@pytest.mark.parametrize("random_seed", RANDOM_SEEDS)
def test_feed_forward_derivative_nn_linear(device, make_kernel, random_seed):
